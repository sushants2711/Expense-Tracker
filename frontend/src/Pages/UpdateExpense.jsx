...
import { handleCreateApiForUpdate } from '../API/ExpenseUpdateApi';
...
export const UpdateExpense = () => {
  ...
  useEffect(() => {
    document.title = "Update-Expense-Page";
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.title && !formData.category && !formData.amount) {
      return handleError("At least one field are required");
    };
    ...
    try {
      const result = await handleCreateApiForUpdate(id, formData);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
      <div className="w-full max-w-md bg-white p-6 rounded-2xl shadow-lg">
        <h2 className="text-2xl font-semibold text-center mb-6 text-blue-600">Update Your Expense</h2>
